...
import (
	...
	"github.com/hyperbolic-timechamber/data-structures-go/src/bst"
)
...
func verifyBSTProperty(t *bst.BinarySearchTree[int]) bool {
...
func TestMinOnEmptyTree(t *testing.T) {
	tree := bst.New[int]()
	_, err := tree.Min()
	if !errors.Is(err, bst.ErrEmptyTree) {
...
func TestRemoveNodeWithLeftChild(t *testing.T) {
	...
	if !tree.Contains(50) || !tree.Contains(20) {
		t.Fatal("expected to contain 50 and 20")
	}
	if !verifyBSTProperty(tree) {
		t.Fatal("BST property violated")
	}
...
func TestRemoveNodeWithRightChild(t *testing.T) {
	...
	if !tree.Contains(50) || !tree.Contains(40) {
		t.Fatal("expected to contain 50 and 40")
	}
	if !verifyBSTProperty(tree) {
		t.Fatal("BST property violated")
	}
...
func TestSortedInsertDegeneratesToList(t *testing.T) {
	tree := bst.New[int]()
	for i := 1; i <= 10; i++ {
		tree.Insert(i)
	}
